...
async function handler(): Promise<void> {
  ...
  try {
    ...
    const levelMakers = Object.keys(levels);
    if (!levelMakers.length) {
      process.stdout.write(`failed!  No maker levels.\n`);
      process.exit(0);
    }

    for (const lm of levelMakers) {
      const mPairs = levels[lm];
      if (!mPairs || !mPairs.length) {
        process.stdout.write(`failed!  No levels for ${lm}.\n`);
        process.exit(0);
      }
      ...
      for (const entry of mPairs) {
        ...
        if (!levels.length) {
          process.stdout.write(
            ` No levels for ${lm} on ${entry.pair.baseTokenName}-${entry.pair.quoteTokenName}. Continuing with next pair...\n`
          );
          continue;
        }
        ...
        makerLevels[lm]!.push({ baseToken, quoteToken, levels });
      }
    }
  } catch (err) {
    process.stdout.write(`failed!  ${(err as Error).toString()}\n`);
